...
新增 office / image 类型在归档前要调 `scripts/convert-office.mjs`,依赖 2 个外部工具(按用途):

| 类型 | 依赖 | 缺失行为 |
|---|---|---|
| `.pptx` / `.docx` / `.xlsx` | `anydoc`(npm `@firecrawl/anydoc`,Rust 写的本地转换器,GFM 输出) | **skill 中止**,要求 `npm i -g @firecrawl/anydoc`;脚本自动 fallback 到 `npx -y @firecrawl/anydoc`,临时拉取即可,无需 API key |
...
| `Inbox/**/*.{docx,DOC}` | Word(递归) | `mv` 到 `01_知识库/<主题目录>/` | 同 pptx(走 anydoc) |
| `Inbox/**/*.{xlsx,XLS}` | Excel(递归) | `mv` 到 `01_知识库/<主题目录>/` | 同 pptx(走 anydoc,自动按 sheet 分段生成 md 表格) |
