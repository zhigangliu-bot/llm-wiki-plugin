...
> 权威源见 [`00_模板/标签词表.md §3`](../../00_模板/标签词表.md)。本表已收编至词表，myconfig 不再维护副本。
...
> 权威源见 [`00_模板/标签词表.md §4`](../../00_模板/标签词表.md)。本表已收编至词表，myconfig 不再维护副本。
...
> 权威源见 [`00_模板/标签词表.md §5`](../../00_模板/标签词表.md)。本节已收编至词表，myconfig 不再维护副本。
>
...
CLAUDE.md 铁律 #3：对话中产生的高价值问答必须**提议**写回 `02_读书笔记/`，但必须由用户显式确认后才可写入。
...
> 「本次回答含 N 个可复用事实点，建议写回 wiki（如 `<路径>`）。是否写回？输入「写」我立刻写。」
